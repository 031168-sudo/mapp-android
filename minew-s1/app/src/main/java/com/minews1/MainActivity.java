...
public class MainActivity extends Activity {
    ...
    private Found identify(ScanResult r){String mac;try{mac=r.getDevice().getAddress();}catch(SecurityException e){return null;}if(mac==null)return null;String name=r.getScanRecord()==null?null:r.getScanRecord().getDeviceName();if(name==null||name.trim().isEmpty())try{name=r.getDevice().getName();}catch(SecurityException ignored){}if(name==null)name="BLE устройство";byte[] md=r.getScanRecord()==null?null:r.getScanRecord().getManufacturerSpecificData(0x0157);byte[] xd=r.getScanRecord()==null?null:r.getScanRecord().getServiceData(XIAOMI_UUID);byte[] mine=r.getScanRecord()==null?null:r.getScanRecord().getServiceData(MINEW_UUID);if(mine!=null)return new Found(mac,name.toLowerCase(Locale.US).contains("minew")?name:"Minew S1","minew",r.getRssi());if(isXiaomi(name,md,xd))return new Found(mac,"Xiaomi LYWSDCGQ/01ZM","xiaomi",r.getRssi());return null;}
    private boolean isXiaomi(String name,byte[] md,byte[] xd){if(name!=null&&name.toUpperCase(Locale.US).contains("LYWSDCGQ"))return true;if(md!=null&&md.length>=8&&u(md,0)==0x50&&u(md,1)==0x20&&u(md,2)==0xaa&&u(md,3)==0x01)return true;return xd!=null&&xd.length>0;}
    ...
    private void parseMeasurement(ScanResult r){if(r.getScanRecord()==null)return;String mac;try{mac=r.getDevice().getAddress();}catch(SecurityException e){return;}DeviceDb.Device d=deviceDb.find(mac);if(d==null)return;float t=Float.NaN,h=Float.NaN;int battery=-1;byte[] data;
        if("minew".equals(d.type)){data=r.getScanRecord().getServiceData(MINEW_UUID);if(data==null||data.length<13||(data[0]&255)!=0xA1||(data[1]&255)!=0x01)return;int tr=(short)(((data[3]&255)<<8)|(data[4]&255));int hr=(short)(((data[5]&255)<<8)|(data[6]&255));t=tr/256f;h=hr/256f;String embedded=String.format(Locale.US,"%02X:%02X:%02X:%02X:%02X:%02X",data[12]&255,data[11]&255,data[10]&255,data[9]&255,data[8]&255,data[7]&255);if(!d.id.equalsIgnoreCase(mac)&&!d.id.equalsIgnoreCase(embedded))return;}
        else if("xiaomi".equals(d.type)){byte[] md=r.getScanRecord().getManufacturerSpecificData(0x0157);if(md==null)return;String hex=hex(md);int pos=hex.indexOf("5020AA01");if(pos<0)return;int off=pos/2+4;if(md.length<off+14)return;int ident=md[off+7]&255;if(ident==0x0D){int tr=(short)((md[off+10]&255)|((md[off+11]&255)<<8));int hr=(short)((md[off+12]&255)|((md[off+13]&255)<<8));t=tr/10f;h=hr/10f;}else if(ident==0x0A){battery=md[off+10]&255;}else return;}
        final float ft=t,fh=h;final int fb=battery;main.post(()->updateSensor(d.id,ft,fh,fb));}
