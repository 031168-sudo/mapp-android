...
public class MainActivity extends Activity {
    ...
    private static final String WTS300_MAC="78:05:41:01:E6:36";
    ...
    private void updateCard(String id){if(deviceCards==null)return;SensorState s=states.get(id);if(s==null)return;for(int i=0;i<deviceCards.getChildCount();i++){View child=deviceCards.getChildAt(i);if(!(child instanceof LinearLayout))continue;LinearLayout c=(LinearLayout)child;TextView vals=(TextView)c.getChildAt(1),status=(TextView)c.getChildAt(3);if(!id.equals(vals.getTag()))continue;if(Float.isNaN(s.t)) vals.setText("—.— °C    —.— % RH"); else if(Float.isNaN(s.h)) vals.setText(String.format(Locale.US,"%.2f °C    —.— % RH",s.t)); else vals.setText(String.format(Locale.US,"%.2f °C    %.1f %% RH",s.t,s.h));status.setText(s.last>0?"●  BLE • "+new SimpleDateFormat("HH:mm:ss",Locale.getDefault()).format(new Date(s.last))+(s.battery>=0?"  •  "+s.battery+"%":""):"Ожидание данных");status.setTextColor(s.last>0?Color.rgb(45,135,80):Color.rgb(110,110,115));}}
    ...
    private void parseMeasurement(ScanResult r){if(r.getScanRecord()==null)return;String scanMac;try{scanMac=r.getDevice().getAddress();}catch(SecurityException e){return;}DeviceDb.Device d=deviceDb.find(scanMac);byte[] xdata=r.getScanRecord().getServiceData(XIAOMI_UUID);byte[] mdata=r.getScanRecord().getServiceData(MINEW_UUID);byte[] edata=r.getScanRecord().getServiceData(EDDYSTONE_UUID);if(edata!=null&&edata.length>=6&&u(edata,0)==0x20){DeviceDb.Device wd=deviceDb.find(WTS300_MAC);if(wd!=null&&"minew_wts300".equals(wd.type))d=wd;}if(xdata!=null&&isXiaomi(xdata)){String embedded=xiaomiMac(xdata);DeviceDb.Device xd=deviceDb.find(embedded);if(xd!=null&&"xiaomi".equals(xd.type))d=xd;}if(d==null&&mdata!=null){String embedded=minewEmbeddedMac(mdata);if(embedded!=null){DeviceDb.Device md=deviceDb.find(embedded);if(md!=null&&"minew".equals(md.type))d=md;}}if(d==null)return;float t=Float.NaN,h=Float.NaN;int battery=-1;
        if("minew".equals(d.type)){byte[] data=mdata;if(data==null||data.length<13||(data[0]&255)!=0xA1||(data[1]&255)!=0x01)return;int tr=(short)(((data[3]&255)<<8)|(data[4]&255));int hr=(short)(((data[5]&255)<<8)|(data[6]&255));t=tr/256f;h=hr/256f;String embedded=String.format(Locale.US,"%02X:%02X:%02X:%02X:%02X:%02X",data[12]&255,data[11]&255,data[10]&255,data[9]&255,data[8]&255,data[7]&255);if(!d.id.equalsIgnoreCase(scanMac)&&!d.id.equalsIgnoreCase(embedded))return;}
        else if("xiaomi".equals(d.type)){if(!isXiaomi(xdata))return;int type=u(xdata,11);if(type==0x0D){if(xdata.length<18)return;int tr=(short)(u(xdata,14)|(u(xdata,15)<<8));int hr=(short)(u(xdata,16)|(u(xdata,17)<<8));t=tr/10f;h=hr/10f;}else if(type==0x0A){if(xdata.length<15)return;battery=u(xdata,14);}else return;} else if("minew_wts300".equals(d.type)){byte[] edata=r.getScanRecord().getServiceData(EDDYSTONE_UUID);if(edata==null||edata.length<6||u(edata,0)!=0x20)return;int tr=(short)((u(edata,4)<<8)|u(edata,5));t=tr/256f;}
