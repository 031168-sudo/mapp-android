...
public class HistoryDb extends SQLiteOpenHelper {
    ...
    public List<Reading> since(long from){return range(from,Long.MAX_VALUE);}
    public List<Reading> range(long from,long to){List<Reading> r=new ArrayList<>();Cursor c=getReadableDatabase().query("readings",new String[]{"ts","temp","hum"},"ts>=? AND ts<?",new String[]{Long.toString(from),Long.toString(to)},null,null,"ts ASC");try{while(c.moveToNext())r.add(new Reading(c.getLong(0),c.getFloat(1),c.getFloat(2)));}finally{c.close();}return r;}
}
