...
# Proven mechanism: server update -> signalStreams -> Status subscription.
old = 'onNewData=function(e){var t=updateUserData(e.userStream),n=pushSignalData(e.signalStreams);(t||n)&&fireNotifications()}'
new = 'onNewData=function(e){var t=updateUserData(e.userStream),n=pushSignalData(e.signalStreams);n&&exp.userDataUpdatedSubscription.publish(exp.getUserData()),(t||n)&&fireNotifications()}'
if old not in s:
    raise SystemExit("onNewData pattern not found")
...
window.__statusDiagRender=function(){var d=window.__statusDiag,el=document.getElementById("statusDiag");if(el)el.textContent="Диагностика: update="+d.updates+" | ответ="+d.responses+" | signalStreams="+d.signals+" | Status="+d.status+" | CID="+d.cid+" | "+d.last;};
'''
if "window.__statusDiag=window.__statusDiag||" not in s:
    if marker not in s:
        raise SystemExit("production marker not found")
...
if old not in s:
    raise SystemExit("update pattern not found")
s = s.replace(old, new, 1)
...
if old not in s:
    raise SystemExit("response pattern not found")
s = s.replace(old, new, 1)

old = 'onNewData=function(e){var t=updateUserData(e.userStream),n=pushSignalData(e.signalStreams);n&&exp.userDataUpdatedSubscription.publish(exp.getUserData()),(t||n)&&fireNotifications()}'
new = 'onNewData=function(e){var t=updateUserData(e.userStream),n=pushSignalData(e.signalStreams);n&&(window.__statusDiag.signals++,window.__statusDiag.last="signalStreams "+moment().format("HH:mm:ss"),window.__statusDiagRender(),exp.userDataUpdatedSubscription.publish(exp.getUserData())),(t||n)&&fireNotifications()}'
if old not in s:
    raise SystemExit("patched onNewData pattern not found")
s = s.replace(old, new, 1)
...
if old not in s:
    raise SystemExit("status attach pattern not found")
s = s.replace(old, new, 1)
...
if old not in s:
    raise SystemExit("status callback pattern not found")
s = s.replace(old, new, 1)

p.write_text(s, encoding="utf-8")
print("OK: diagnostic + signal-to-Status refresh patch applied")
